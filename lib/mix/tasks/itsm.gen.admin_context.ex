defmodule Mix.Tasks.Itsm.Gen.AdminContext do
  ...
  def build(args, opts \\ []) do
    ...
    schema =
      Mix.Tasks.Itsm.Gen.AdminSchema.build([schema_module, plural | schema_args], opts, help)
  ...
  def files_to_be_generated(%Context{schema: schema}) do
    if schema.generate? do
      Mix.Tasks.Itsm.Gen.AdminSchema.files_to_be_generated(schema)
    else
      []
    end
  end
  ...
  def copy_new_files(%Context{schema: schema} = context, paths, binding) do
    if schema.generate?, do: Mix.Tasks.Itsm.Gen.AdminSchema.copy_new_files(schema, paths, binding)
  ...
  def ensure_context_file_exists(%Context{file: file} = context, paths, binding) do
    unless Context.pre_existing?(context) do
      Mix.Generator.create_file(
        file,
        Mix.Phoenix.eval_from(paths, "priv/templates/itsm.gen.context/context.ex.eex", binding)
      )
    end
  end
  ...
  defp inject_schema_access(%Context{file: file} = context, paths, binding) do
    ...
    |> Mix.Phoenix.eval_from(
      "priv/templates/itsm.gen.context/#{schema_access_template(context)}",
      binding
    )
  ...
  def print_shell_instructions(%Context{schema: schema}) do
    if schema.generate? do
      Mix.Tasks.Itsm.Gen.AdminSchema.print_shell_instructions(schema)
    else
      :ok
    end
  end
